...
const FinancialSection = ({ mode = "full" }) => {
  ...
  return (
    <section
      ...
      onClick={
        isPreview ? () => navigate("/admin/financial-analytics") : undefined
      }>
      {/* ── PREVIEW HEADER ─────────────────────────────────────────────── */}
      {isPreview && (
        <div
          ...
          <div className="flex md:grid md:grid-cols-4 gap-4 md:gap-6 snap-x snap-mandatory md:snap-none md:px-0">
            ...
            <div className="min-w-[60vw] md:min-w-0 snap-start">
              <FinancialKpiCard
                ...
                delta={calcDelta(
                  totals.payoutsProcessed,
                  totals.prevPayoutsProcessed,
                )}
                compareLabel={getCompareLabel()}
      ...
      {!isPreview && (
        <RecentPayoutsTable
          ...
          page={page}
          setPage={setPage}
          totalCount={totals.totalTransactions}
          statusFilter={statusFilter}
          onStatusChange={handleStatusChange}
          searchQuery={searchQuery}
