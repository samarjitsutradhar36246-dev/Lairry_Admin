...
import { Search, ArrowLeft, ArrowRight, Plus, Download, Mail, Ban, Building2, Filter } from "lucide-react";
...
import { sendWelcomeEmail, sendBulkEmails } from "../../common/services/emailService";
...
const InstitutesSection = ({ mode = "full" }) => {
  ...
  const applyFilters = useCallback(
    (query, filters = {}) => {
      ...
      let ascending = false;
      switch (sort) {
        case "name-asc":   orderBy = "institute_name"; ascending = true;  break;
        case "name-desc":  orderBy = "institute_name"; ascending = false; break;
        case "revenue-asc":  orderBy = "revenue"; ascending = true;  break;
        case "revenue-desc": orderBy = "revenue"; ascending = false; break;
      }
...
const openEmailModal = () => {
  ...
  const newRecipients = selectedInstitutesData
    .map((i) => ({ name: i.institute_name, email: i.institute_email }));
...
const handleSendBulkEmail = async (emails, subject, body) => {
  ...
  if (isAllMatchingSelected) {
    try {
      ...
      NotificationService.error("Could not resolve recipients: " + err.message);
      return;
    }
  } else {
  ...
  try {
    ...
    NotificationService.success(`Email sent to ${resolvedEmails.length} recipient(s)`);
    setEmailModalOpen(false);
    resetSelection();
      setSelectedEmails([]);
  setEmailSubject("");
...
const newRecipients = selectedInstitutesData.map((i) => ({
  ...
  const exportCSV = () => {
    if (!institutes.length) { NotificationService.error("No institutes to export!"); return; }
    const headers = ["ID", "Name", "City", "Revenue", "Status", "Email", "Notes"];
    const sanitize = (v) => String(v ?? "").replace(/\r?\n|\r/g, " ");
    const rows = institutes.map((i) => [
      sanitize(i.institute_id), sanitize(i.institute_name), sanitize(i.location_city),
      sanitize(i.revenue), sanitize(i.account_status), sanitize(i.institute_email), sanitize(i.admin_notes),
    ]);
  ...
  const resetForm = () => { setNewInstitute(initialInstituteState); setErrors({}); };
  ...
  const handleSaveNewInstitute = async () => {
    ...
    if (Object.keys(validationErrors).length > 0) { setErrors(validationErrors); setIsSaving(false); return; }
    const emailExists = institutes.some((inst) => inst.institute_email === newInstitute.institute_email);
    if (emailExists) { setErrors({ institute_email: "This email is already used" }); setIsSaving(false); return; }

    try {
      const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*()";
      const tempPassword = Array.from({ length: 10 }, () => chars[Math.floor(Math.random() * chars.length)]).join("");

      const { data, error } = await supabase.from("institutes").insert([{
      ...
      await supabase.rpc("update_kpi", { kpi_name: "total_institutes", delta: 1 });
      ...
      sendWelcomeEmail(newInstitute.institute_email, newInstitute.institute_display_name || newInstitute.institute_name, tempPassword);
      resetForm();
      setAddOpen(false);
    } catch (err) {
  ...
  const handleSaveEdit = async (updatedInstitute) => {
    ...
    const keysToCheck = [
      "institute_name","institute_display_name","institute_type","institute_email",
      "support_email","contact_person_name","contact_person_designation",
      "contact_phone","contact_phone_alt","location_city","location_state",
      "location_country","location_pin","account_status","revenue","admin_notes",
    ];
    const changes = {};
    keysToCheck.forEach((key) => {
      if (updatedInstitute[key] !== undefined && updatedInstitute[key] !== originalInstitute[key])
        changes[key] = updatedInstitute[key];
    });
    if (!Object.keys(changes).length) { NotificationService.error("No changes detected!"); return; }

    try {
      changes.updated_at = new Date().toISOString();
      const { error } = await supabase.from("institutes").update(changes).eq("id", updatedInstitute.institute_id);
      if (error) throw error;
      setInstitutes((prev) =>
        prev.map((inst) =>
          inst.institute_id === updatedInstitute.institute_id ? { ...inst, ...changes } : inst
        )
      );
      setModalOpen(false);
  ...
  const getChangedFields = () => {
    ...
    const changes = {};
    keysToCheck.forEach((key) => { if (selected[key] !== originalInstitute[key]) changes[key] = true; });
    return changes;
  };
...
useEffect(() => {
  ...
  return (
    <section>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {isPreview && (
          <div className="flex justify-between items-center p-6 rounded-2xl 
            ...
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Institutes Overview</h3>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); navigate("/admin/institutes-management"); }}
        ...
        {/* HEADER */}
        {!isPreview && (
          <div id="tour-institutes-header" className="relative overflow-hidden rounded-2xl 
            bg-gradient-to-br from-white to-gray-50 dark:from-slate-900/90 dark:to-slate-800/90 
        ...
        {/* SEARCH + SORT + EXPORT */}
        <div id="tour-institutes-search" className="relative overflow-hidden rounded-2xl 
          bg-gradient-to-br from-white to-gray-50 dark:from-slate-900/90 dark:to-slate-800/90 
          ...
          <div className="relative flex flex-col lg:flex-row gap-3 sm:gap-4 items-stretch lg:items-center lg:justify-between">
            <div className="relative flex-1 max-w-full lg:max-w-md">
              <Search size={18} className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-slate-400" />
              <input
                value={search}
            ...
            <div className="flex gap-2 sm:gap-3 items-center overflow-x-auto pb-2 lg:pb-0 -mx-3 px-3 sm:mx-0 sm:px-0 lg:overflow-visible scrollbar-hide">
              <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl 
                ...
                <Filter size={14} className="sm:w-4 sm:h-4 text-purple-600 dark:text-purple-400" />
                ...
                >
                  ...
                  <option value="revenue-asc" className="bg-white dark:bg-slate-800">Revenue ↑</option>
                  <option value="revenue-desc" className="bg-white dark:bg-slate-800">Revenue ↓</option>
                </select>
              </div>
        ...
        {(selectedIds.length > 0 || isAllMatchingSelected) && (
          <div id="tour-institutes-bulk" className="relative overflow-hidden rounded-2xl 
            bg-gradient-to-br from-white to-gray-50 dark:from-slate-900/90 dark:to-slate-800/90 
  ...
  <span className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">
    All {totalMatchingCount.toLocaleString()} matching institutes selected
  </span>
  ...
  <span className="text-sm font-semibold text-cyan-600 dark:text-cyan-400">
    {selectedIds.length} of {institutes.length} on this page selected
  </span>
)}
                  </div>
                  ...
                  {!isAllMatchingSelected &&
                    ...
                    <button
                      ...
                      className="text-sm font-medium text-cyan-600 dark:text-cyan-400 
                        ...
                        transition-colors duration-150 cursor-pointer whitespace-nowrap"
                    >
                      Select all {totalMatchingCount.toLocaleString()} institutes
                    </button>
                  )}
              ...
              <div className="relative flex flex-wrap gap-3 items-center">
                ...
                >
                  ...
                  <span className="text-sm">
                    Suspend {isAllMatchingSelected ? `All ${totalMatchingCount.toLocaleString()}` : selectedIds.length}
                  </span>
                </button>
...
<div className="relative inline-flex items-center group/container">
  ...
  {!emailModalOpen && (() => {
    try {
      const d = JSON.parse(localStorage.getItem("email_draft_institutes") || "{}");
...
<div className="relative inline-flex items-center group/container">
  ...
  {!notificationModalOpen && (() => {
    try {
      const d = JSON.parse(localStorage.getItem("notification_draft_institutes") || "{}");
...
/>
        ...
        <EditInstituteModal
          ...
          onClose={() => { setModalOpen(false); setOriginalInstitute(null); }}
          onChange={(field, value) => setSelected((prev) => ({ ...prev, [field]: value }))}
  ...
  getRecipientInitial={(r) => r.name?.[0]?.toUpperCase() || r.email?.[0]?.toUpperCase() || "I"}
  onDiscard={handleDiscardEmail}        
  draftKey="email_draft_institutes"
activeEmailSet={new Set(selectedInstitutesData.map((i) => i.institute_email))}
        />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl 
            ...
            <div className="text-center space-y-2">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Suspend Institutes?</h3>
              <p className="text-sm text-gray-600 dark:text-slate-400 leading-relaxed">
                ...
                . Their accounts will be deactivated immediately. This can be undone by editing each institute individually.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => { setSuspendConfirmOpen(false); blockSelected(); }}
                className="flex-1 px-4 py-2.5 rounded-xl 
