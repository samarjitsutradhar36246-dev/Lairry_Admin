...
import { ArrowLeft, Mail, Phone, GraduationCap, Calendar, CheckCircle, Clock, Edit3, User } from "lucide-react";
...
const handleSaveStudent = async (updatedStudent) => {
  // 1. Check if any field actually changed
  const hasChanges =
  ...
  try {
    ...
    setStudent(updatedStudent); // update main state
    setEditModalOpen(false);
    setSelectedStudent(null);
...
const hasChanges = Object.keys(changedFields).length > 0;
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-600 dark:text-slate-400 text-lg">Loading student data...</p>
        </div>
      </div>
    );
  ...
  if (!student)
    return (
      <div className="flex items-center justify-center min-h-screen p-6">
        <p className="text-red-600 dark:text-red-400 text-xl">Student data not available.</p>
      </div>
    );
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="flex-1 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-gray-600 dark:text-slate-400 flex items-center gap-2 text-lg">
                      <Mail size={18} className="text-cyan-500 dark:text-cyan-400 flex-shrink-0" />
                ...
                <div className="flex items-center gap-3 pt-2">
                  {student.has_completed_setup === "YES" ? (
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3 p-3 rounded-xl 
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wider mb-1">Email</p>
                    <p className="text-gray-900 dark:text-white font-medium break-all">{student.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 rounded-xl 
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wider mb-1">Phone Number</p>
                    <p className="text-gray-900 dark:text-white font-medium">{student.phone_number}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="relative p-6 space-y-4">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3 p-3 rounded-xl 
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wider mb-1">Qualification</p>
                    <p className="text-gray-900 dark:text-white font-medium">{student.qualification}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 rounded-xl 
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wider mb-1">Joined Date</p>
                    <p className="text-gray-900 dark:text-white font-medium">{student.joined}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="relative overflow-hidden rounded-2xl 
          ...
          <div className="relative p-6">
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl 
              ...
              <div>
                <p className="text-xs text-gray-500 dark:text-slate-500 font-medium uppercase tracking-wider mb-1">Setup Status</p>
                <p className={`text-lg font-semibold ${
                  student.has_completed_setup === "YES" 
                    ? "text-green-600 dark:text-green-400" 
                    : "text-yellow-600 dark:text-yellow-400"
                }`}>
                  {student.has_completed_setup === "YES" ? "Completed" : "Pending"}
                </p>
              </div>
            </div>
          </div>
