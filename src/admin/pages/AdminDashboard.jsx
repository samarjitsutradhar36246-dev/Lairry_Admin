...
const fetchKpis = useCallback(async (isInitial = false) => {
  if (isInitial) setDataReady(false); // ← reset on each new fetch
    ...
    setKpiData({ ...currentKpis, delta, timestamp: new Date().toISOString() });
    setDataReady(true); // ← fires AFTER data is set, triggers scroll restore
  }, [deltaPeriod]); // ← dataReady NOT in deps, prevents loop
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="relative flex items-start md:items-center justify-between gap-2">
        ...
        <div id="tour-delta-dropdown" className="relative inline-block text-left flex-shrink-0">
          ...
          {showDropdown && (
            <div className="origin-top-right absolute right-0 mt-2 w-32 md:w-40 rounded-md 
      ...
      {/* KPI Cards - First Row */}
      <div id="tour-kpi-row-1" className="md:grid md:grid-cols-4 md:gap-6 overflow-x-auto md:overflow-x-visible">
        <div className="flex md:contents gap-4 md:gap-0 pb-4 md:pb-0">
          ...
          <div className="flex-shrink-0 w-[280px] md:w-auto">
            <KpiCard
              title="Exam Listed"
              value={kpiData.examListed}
              delta={`${kpiData.delta?.examListed || 0}`}
              color="purple"
            />
          </div>
          <div className="flex-shrink-0 w-[280px] md:w-auto">
            <KpiCard
              title="Queries Raised (demo data)"
              value="20"
              delta="stable"
              color="green"
            />
      ...
      <div id="tour-kpi-row-2" className="md:grid md:grid-cols-4 md:gap-6 overflow-x-auto md:overflow-x-visible">
        <div className="flex md:contents gap-4 md:gap-0 pb-4 md:pb-0">
          <div className="flex-shrink-0 w-[280px] md:w-auto">
            <KpiCard
          ...
          <div className="flex-shrink-0 w-[280px] md:w-auto">
            <KpiCard title="Total Feedbacks (demo data)" value="20k" delta="+5%" color="blue" />
          </div>
          <div className="flex-shrink-0 w-[280px] md:w-auto">
            <KpiCard title="Bad Reviews (demo data)" value="100" delta="-2%" color="red" />
          </div>
        </div>
      </div>
